...
export async function POST(
  ...
  if (
    ...
    return Response.json(
      { error: 'Expected a JSON body with string "field" and "value"' },
      { status: 400 },
    );
  }
  ...
  try {
    const outcome = await new PostgresItemDetailRepository().correctFact(
      id,
      field,
      value,
    );
    ...
    console.error(
      'Failed to correct a fact',
      error instanceof Error ? error.name : 'UnknownError',
    );
    return Response.json(
      { error: 'Could not save this correction' },
      { status: 500 },
    );
  }
}
