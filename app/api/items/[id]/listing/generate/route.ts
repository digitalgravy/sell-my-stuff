...
export async function POST(
  ...
  try {
    const outcome = await new PostgresItemDetailRepository().generateListingDraft(id);
    if (!outcome.ok) {
    ...
    console.error(
      'Failed to generate a listing draft',
      error instanceof Error ? error.name : 'UnknownError',
    );
    return Response.json(
      { error: 'Could not generate a listing draft' },
      { status: 500 },
    );
  }
}
